...
######### Instructions  ############
## Purpose: The purpose of this script is to map the cleaned RNASeq data (.fasta) from step 2 of the core pipeline, outlined in script "trim_FastQC.sh," to the reference genome. 
## For the core pipeline, the reference genome used was ASM211347v1, curated by Arizona State University (2021). 
## For the reference genome effect study, which is outlined in this adapted script, a substitute reference genome is being used... PA42v3.0 curated by Indiana State University Bloomington (2016)
## Thus, the inputs will be the same as the core mapping script... i.e. the same input trimmed .fastq files... except the PA42 reference genome and annotations will be used instead.
...
REFD=/scratch/$MyID/PracticeRNAseq/DaphniaRefGenome_PA42         ##This directory contains the indexed reference genome for Daphnia Pulex and corresponding annotations; DaphniaRefGenome_PA42.fasta + DaphniaRefGenome_PA42.gff3
MAPD=/scratch/$MyID/PracticeRNAseq/Map_HiSat2_PA42               ##This is where the mapped sequence files will be located (.sam and .bam)
COUNTSD=/scratch/$MyID/PracticeRNAseq/Counts_StringTie_PA42      ##This is where the read counts files will be located alongside other stringtie outputs
RESULTSD=/home/aubclsb0329/PracticeRNAseq/Counts_H_S_PA42        ##This is where the results you want to be brought back to your computer are

REF=DaphniaRefGenome_PA42                  ## This is what the "easy name" will be for the genome reference and annotation
...
cd $REFD
cp /scratch/team3_finalproj/RefGenome/DaphniaRefGenome_PA42.fasta .
cp /scratch/team3_finalproj/RefGenome/DaphniaRefGenome_PA42.gff3 . 
...
hisat2-build --ss $REF.ss --exon $REF.exon $REF.fasta Dpul_PA42_index
...
cd $COUNTSD
python /scratch/team3_finalproj/original_scripts/prepDE.py -i $COUNTSD
